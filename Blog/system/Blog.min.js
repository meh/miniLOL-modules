(function(){return Class.create({initialize:function(a,e,c){this.root=a;var b=this;this.resource=new miniLOL.Resource("Blog",{load:function(d){var g=this.data;new Ajax.Request(d+"?failCache="+Math.random(),{method:"get",asynchronous:false,requestHeaders:{"Cache-Control":"must-revalidate",Pragma:"no-cache"},onSuccess:function(f){if(!Document.check(f.responseXML,d))g.data=Document.fix(f.responseXML)},onFailure:function(f){miniLOL.error("Error while loading Blog's data.xml (#{error})".interpolate({error:f.status}))}});
