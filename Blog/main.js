...
miniLOL.module.create('Blog', {
    ...
    initialize: function () {
        this.Blog = miniLOL.utils.execute(this.root+'/system/Blog.min.js');
    ...
    execute: function (args) {
        if (args['post'] || args['edit'] || args['remove']) {
            ...
            if (args['comment']) {
                if (args['post']) {
                    if (args['do']) {
                        ...
                        miniLOL.content.set(this.blog.template.evaluate('new_comment', { id: args['id'] }));
                    }
                }
            }
            else {
                if (args['post']) {
                    if (args['do']) {
                        ...
                        miniLOL.content.set(this.blog.template.evaluate('new_post', { author: miniLOL.config['Blog'].author.post }));
                    }
                }
                else if (args['edit']) {
                    if (args['id']) {
                        if (args['do']) {
                            ...
                            if (post == null) {
                                miniLOL.error('The post does not exist.', miniLOL.theme.content(), true);
                                return false;
                            }
        
                            miniLOL.content.set(this.blog.template.evaluate('edit_post', {
            ...
            miniLOL.content.set(this.blog.template.evaluate('dashboard'));
        }
        else {
            ...
            if (args['id']) {
                ...
                if (post) {
                    ...
                    miniLOL.content.set(this.blog.template.evaluate('post', { post: post, number: posts.indexOf(post) + 1, total: posts.length }));
                ...
                if (args['number'] <= posts.length) {
                    ...
                    miniLOL.content.set(this.blog.template.evaluate('post', { post: post, number: parseInt(args['number']), total: posts.length }));
                ...
                miniLOL.content.set(this.blog.template.evaluate('posts', { posts: posts, page: parseInt(args['page']), total: Math.ceil(this.blog.getPosts().length / miniLOL.config['Blog'].postsPerPage) }));
            }
        }
