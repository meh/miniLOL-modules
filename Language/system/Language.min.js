(function(){return Class.create({initialize:function(a,c){this.root=a;var e=this;this.resource=new miniLOL.Resource("Language.resource",{load:function(d){e.languages=this.data;new Ajax.Request(d,{method:"get",asynchronous:false,onSuccess:function(b){miniLOL.Document.check(b.responseXML,d)||$A(b.responseXML.getElementsByTagName("language")).each(function(f){e.languages.push({code:f.getAttribute("code"),name:f.getAttribute("name")})})},onFailure:function(b){miniLOL.error("Error while loading config.xml (#{error})".interpolate({error:b.status}))}})},
