Ajax.Request.addMethods({request:function(a){this.url=a;this.method=this.options.method;if(Object.isString(this.options.parameters))this.options.parameters=this.options.parameters.toQueryParams();if(this.options.minified){a=this.url.replace(/\.([^.]*?)$/,".min.\u0001");if(miniLOL.utils.exists(a))this.url=a}if(this.options.tokenized)this.options.parameters.__token=miniLOL.module.execute("Security",{token:true});a=Object.toQueryString(this.options.parameters);if(!["get","post","head"].include(this.method)){a+=
...
this.transport.onreadystatechange=this.onStateChange.bind(this);this.setRequestHeaders();this.body=this.method=="post"?this.options.postBody||a:null;this.transport.send(this.body);!this.options.asynchronous&&this.transport.overrideMimeType&&this.onStateChange()}catch(c){this.dispatchException(c)}}});
