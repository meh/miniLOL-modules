(function(){return Class.create({initialize:function(a,c){this.root=a;var d=this;this.resource=new miniLOL.Resource("Menu Shell",{load:function(e){d._commands=this._data;new Ajax.Request(e,{method:"get",asynchronous:false,onSuccess:function(b){if(!miniLOL.Document.check(b.responseXML,e)){b=b.responseXML.documentElement.getElementsByTagName("command");for(var f=0;f<b.length;f++){var g=b[f].getAttribute("name");try{d._commands[g]=new Function(b[f].firstChild.nodeValue);d._commands[g].length=b[f].getAttribute("length")}catch(h){miniLOL.error("Error while creating `#{name}` wrapper from #{path}:<br/><br/>#{error}".interpolate({name:g,
