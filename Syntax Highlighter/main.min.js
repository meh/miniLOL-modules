...
"/resources/styles")}Event.observe(document,":go",function(){miniLOL.module.execute("Syntax Highlighter")});Event.observe(window,"resize",this.doWrap);miniLOL.CSS.create("body .syntaxhighlighter .line { white-space: pre-wrap !important; }")},loadFile:function(a){if(a&&!this.loaded[a])if(miniLOL.utils.include(this.root+"/resources/languages/shBrush"+a+".min.js")){this.loaded[a]=true;SyntaxHighlighter.vars.discoveredBrushes={};for(var d in SyntaxHighlighter.brushes){var b=SyntaxHighlighter.brushes[d].aliases;
b&&b.each(function(c){SyntaxHighlighter.vars.discoveredBrushes[c]=d})}}else $$(SyntaxHighlighter.config.tagName).each(function(c){var e=((c.getAttribute("class")||"").match(/brush:\s*(.*?)(;|$)/)||[])[1];if(e&&this.aliases[e]==a){c.setAttribute("class",c.getAttribute("class").replace(/brush:.*(;|$)/,""));c.update("Could not load the brush.")}},this)},loadTagFiles:function(){$$(SyntaxHighlighter.config.tagName).each(function(a){(a=((a.getAttribute("class")||"").match(/brush:\s*(.*?)(;|$)/)||[])[1])&&
this.loadFile(this.aliases[a])},this)},doWrap:function(){$$(".syntaxhighlighter").each(function(a){var d=a.select("td.gutter").first(),b=a.select("td.code").first();d.select(".line").each(function(c,e){var f=b.select(".line:nth-child(#{index})".interpolate({index:e+1})).first().getHeight();if(f)f+="px";else f="auto";c.setStyle("height: #{height} !important".interpolate({height:f}))})},this)},execute:function(a){if(!$$(".syntaxhighlighter").length){a=a||{};var d=Object.extend({},SyntaxHighlighter.defaults);
for(var b in a)SyntaxHighlighter.defaults[b]=a[b][0]=="["&&a[b][a[b].length-1]=="]"?eval(a[b].replace(/[^\d\[\],]/g,"")):a[b];if(a.lang){SyntaxHighlighter.defaults.brush=a.lang;this.loadFile(this.aliases[a.lang])}this.loadTagFiles();SyntaxHighlighter.highlight();SyntaxHighlighter.defaults=d;this.doWrap()}}});
